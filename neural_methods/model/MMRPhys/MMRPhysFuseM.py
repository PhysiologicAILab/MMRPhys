...
class RGBTFeatureFusion_Fast(nn.Module):
    def __init__(self, dropout_rate=0.1, debug=False):
        ...
        self.debug = debug
        #                                                        Input: #B, 2*nf[1], 180, 36, 36
        self.FeatureFusion = nn.Sequential(
            ConvBlock3D(2*nf[1], nf[2], [3, 3, 3], [1, 1, 1], [1, 0, 0], dilation=[1, 1, 1]), #B, nf[2], 180, 32, 32
            ConvBlock3D(nf[2], nf[2], [3, 3, 3], [1, 2, 2], [1, 0, 0], dilation=[1, 1, 1]), #B, nf[2], 180, 15, 15
...
class RGBTFeatureFusion_Slow(nn.Module):
    def __init__(self, dropout_rate=0.1, debug=False):
        ...
        #                                                        Input: #B, 2*nf[1], 180, 36, 36
        self.FeatureFusion = nn.Sequential(
            ConvBlock3D(2*nf[1], nf[2], [3, 3, 3], [1, 1, 1], [2, 0, 0], dilation=[2, 1, 1]), #B, nf[2], 180, 32, 32
            ConvBlock3D(nf[2], nf[2], [3, 3, 3], [1, 2, 2], [2, 0, 0], dilation=[2, 1, 1]), #B, nf[2], 180, 15, 15
            ConvBlock3D(nf[2], nf[2], [3, 3, 3], [1, 1, 1], [1, 0, 0], dilation=[1, 1, 1]), #B, nf[2], 180, 13, 13
            nn.Dropout3d(p=dropout_rate),
        )
