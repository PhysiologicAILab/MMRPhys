...
class MMRPhysSEFInference:
    def __init__(self, model_path, video_file):
        ...
        self.json_filename = self.video_file.parent.joinpath(self.video_file.stem + '_estimated_signals.json')
        self.plot_filename = self.video_file.parent.joinpath(self.video_file.stem + '_estimated_signals.png')
    ...
    def infer_rphys(self):
        ...
        with torch.no_grad():
            torch_frames = torch.from_numpy(
                self.frame_buffer).float().to(self.device)
...
def main():
    ...
    parser.add_argument('--model_path', type=str, 
                        default='tools/torch2onnx/SCAMPS_Multi_9x9.pth', help='path to model file')
